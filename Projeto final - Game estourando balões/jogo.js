...
function iniciaJogo(){
	...
	if (nivel_jogo == 1) {// 1 facíl -> 120 segundos
		 tempo_segundos = 120;
	};
...
function contagem_tempo(segundos){
	...
	if (segundos == -1) {
		clearTimeout(timerId); //A função do clearTimeout é parar a função setTimeout
		game_over();
		return false;
	};
...
function game_over(){
	alert('Fim de jogo,você não consiguiu estourar todos os balões a tempo');
}
...
function pontuacao(acao){
 ...
 function situacao_jogo(baloes_inteiros){
 	if (baloes_inteiros == 0) {
 		alert('Parabéns, você consiguiu estourar todos os balões a tempo');
 		parar_jogo();
 	};
 }
